...
const Snowflake: React.FC<{ x: number; y: number; size: number; delay: number; color: string }> = ({
  x,
  y,
  size,
  delay,
...
const SnowyIcon: React.FC<{ size?: number; cloudColor?: string; snowflakeColor?: string }> = ({
  size = 80,
  cloudColor = '#CBD5E1',
  snowflakeColor = '#E0F2F7',
}) => (
  <motion.svg
    width={size}
    height={size}
  ...
  >
    ...
    {/* Snowflakes */}
    <Snowflake x={24} y={48} size={2.5} delay={0} color={snowflakeColor} />
    <Snowflake x={32} y={52} size={3} delay={0.5} color={snowflakeColor} />
    <Snowflake x={40} y={46} size={2.8} delay={1} color={snowflakeColor} />
  </motion.svg>
);
